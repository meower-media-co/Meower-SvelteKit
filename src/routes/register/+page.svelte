<script lang="ts">
	...
	async function RegisterCallback() {
		...
		const profile = await (
			await fetch(`${apiUrl}/users/${_username}`)
		).json();
...
<PopupHome title="signup">
	<svelte:fragment slot="content">
		<form on:submit={async () => {await RegisterCallback()}}>
			<input type="text" bind:value={username} />
			<br />
			<input type="password" bind:value={pswd} />
			<br />
			<!-- Agree to TOS -->
			<p><input type="checkbox" bind:checked={agree} />  I agree to the <a href="https://meower.org/legal">Terms of Service</a></p>
			<br />
